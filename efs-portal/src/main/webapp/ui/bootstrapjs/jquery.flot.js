...
(function(B) {
    ...
    function Plot(placeholder, data_, options_, plugins) {
        ...
        function fillInSeriesOptions() {
            ...
            while (colors.length < neededColors) {
                var c;
                if (options.colors.length == i) c = $.color.make(100, 100, 100);
        ...
        function setupTickGeneration(axis) {
            ...
            else noTicks = 0.3 * Math.sqrt(axis.direction == "x" ? canvasWidth: canvasHeight);
            ...
            if (opts.mode == "time") {
                ...
                var minSize = 0;
                if (opts.minTickSize != null) if (typeof opts.tickSize == "number") minSize = opts.tickSize;
                ...
                formatter = function(v, axis) {
                    ...
                    else if (t < timeUnitSize.day) if (span < 2 * timeUnitSize.day) fmt = "%h:%M" + suffix;
                    else fmt = "%b %d %h:%M" + suffix;
                    else if (t < timeUnitSize.month) fmt = "%b %d";
                    else if (t < timeUnitSize.year) if (span < timeUnitSize.year) fmt = "%b";
                    else fmt = "%b %y";
                    else fmt = "%y";
        ...
        function setTicks(axis) {
            ...
            else if (oticks) if ($.isFunction(oticks)) ticks = oticks({
                min: axis.min,
                max: axis.max
            });
            else ticks = oticks;
        ...
        function drawGrid() {
            ...
            for (var j = 0; j < axes.length; ++j) {
                ...
                for (i = 0; i < axis.ticks.length; ++i) {
                    ...
                    if (ctx.lineWidth == 1) if (axis.direction == "x") x = Math.floor(x) + 0.5;
        ...
        function findNearbyItem(mouseX, mouseY, seriesFilter) {
            ...
            for (i = series.length - 1; i >= 0; --i) {
                ...
                if (s.lines.show || s.points.show) for (j = 0; j < points.length; j += ps) {
                    var x = points[j],
                    y = points[j + 1];
                    if (x == null) continue;
    ...
    $.plot.formatDate = function(d, fmt, monthNames) {
        ...
        if (fmt.search(/%p|%P/) != -1) if (hours > 12) hours = hours - 12;
