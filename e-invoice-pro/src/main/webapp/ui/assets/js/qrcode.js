(function($) {
    ...
    Qrcode.prototype = {
		weixReady: function(tempBtn) {
			$(tempBtn).bind('click', this.scanWeixQRCode);
        },
        nativeReady: function(tempBtn) {
            ...
            $('[node-type=jsbridge]').bind('change', this.getImgFile);
        },
		...
		scanWeixQRCode:function(){
			console.log('扫码购物小票二维码');
			wx.scanQRCode({
			  needResult: 1,
			  desc: '扫码购物小票二维码',
			  success: function (data) {
				getQrCode(data.resultStr);
        ...
        getImgFile: function() {
            var _this_ = this;
            var inputDom = $('[node-type=jsbridge]');
