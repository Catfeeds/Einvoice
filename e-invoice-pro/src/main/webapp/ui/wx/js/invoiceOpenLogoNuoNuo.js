...
function companyname() {
    $.ajax({
        url: $.getService("ui", "getEnterpriseByIdTitle"),
        type: "POST",
        dataType: "json",
        contentType: "application/json;charset=UTF-8",
        success: function (data) {
            $("#companyName").text(data.entname);
        }
    })
}

function GetDanweiName() {
    var data = {};
    $("#invoiceHeadEntText").autocomplete({
        source: function (request, response) {
            data.name = $("#invoiceHeadEntText").val();
            $.myAjax({
                url: $.getService("ui", "getInvoicerListByName"),
                ...
                success: function (dataJson) {
                    ...
                    if (dataJson.code == 0 && dataJson.data) {
                        for (var i = 0; i < dataJson.data.length; i++) {
                            myArray[i] = dataJson.data[i].name;
                        }
                        response($.map(dataJson.data, function (item) {
                            ...
                            return {
                                ...
                                taxId: item.taxId,
                                addressAndPhone: item.addressAndPhone,
                                bankAndAccount: item.bankAndAccount
                            }
                        }));
                    }
                }
...
function changeCustomerInfo(data) {
    $("#invoiceHeadEntTaxno").val(data.taxId);
    $("#invoiceHeadEntAddr").val(data.addressAndPhone);
    $("#invoiceHeadEntBank").val(data.bankAndAccount);
}

function keydown() {
    $("#invoiceHeadEntText").on('input', function () {
        if ($("#invoiceHeadEntText").val() == "") {
            $("#invoiceHeadEntTaxno").val("");
            $("#invoiceHeadEntAddr").val("");
            $("#invoiceHeadEntBank").val("");
        }
    });
}
...
$(document).ready(function () {
    ...
    function clickNextbton(sheetidG) {
        $.myAjax({
            url: $.getService("ui", "clickNext"),
            data: sheetidG,
            contentType: "application/json;charset=UTF-8",
            success: function (dataJson) {
...
function getbillInfo(postData) {
    ...
    $.myAjax({
        ...
        success: function (dataJson) {
            ...
            sheetidG = dataJson.data.sheetid;
            ...
            if (dataJson.data.flag == 0) {
                ...
                tradedate = dataJson.data.tradedate;
                var a = "";
                var b = "";
...
/*function changeTab(t) {
    ...
    if (pIndex < nIndex) {
        $("#" + pId).addClass("am-animation-slide-top am-animation-reverse").one($.AMUI.support.animation.end, function () { // 当前table向上滑动
            // 去除当前table的动画效果,并隐藏
        ...
        $("#" + pId).addClass("am-animation-slide-bottom am-animation-reverse").one($.AMUI.support.animation.end, function () { // 当前table向下滑动	去除当前table的动画效果,并隐藏
