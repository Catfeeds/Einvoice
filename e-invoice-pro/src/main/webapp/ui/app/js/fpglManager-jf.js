	...
	if(data.enddate !='' && data.enddate!=null){
		...
		$.myAjax({
			...
			success: function (dataJson) {
				...
				if(dataJson.data.length==0&&flag==0){
					if(sheetid !='' && sheetid!=null){
						flag=1;
						searchByxiaopiao(sheetid);
					}
...
function searchByxiaopiao(billno){
	var isbendian=false;
	var issearch = true;
	var data ={};
	...
	if(issearch){
		$.myAjax({
				...
				if(dataJson.data.flag  == 1 && dataJson.data.iqseqno != null){
					...
					$.myAjax({
						...
						success: function (data) {
							...
							if(user.loginid ==data.data[0].iqperson){
								if(data.data[0].iqfplxdm  !=''  && data.data[0].iqfplxdm != null){
								var globalparam = "<?xml version=\"1.0\" encoding=\"gbk\"?>\r\n<business id=\"20001\" comment=\"参数设置\">\r\n<body yylxdm=\"1\">\r\n<servletip>"+taxinfo.itfkpd+"</servletip>\r\n<servletport>"+taxinfo.itfskpkl+"</servletport>\r\n<keypwd>"+taxinfo.itfkeypwd+"</keypwd>\r\n</body>\r\n</business>";
								//alert(globalparam);
								var searchparm = "<?xml version=\"1.0\" encoding=\"gbk\"?>"+
												"<business id=\"10010\" comment=\"发票查询\">"+
												"<body yylxdm=\"1\">"+
												"<kpzdbs>"+user.kpd+"</kpzdbs>"+
												"<fplxdm>"+data.data[0].iqfplxdm+"</fplxdm>"+
												"<cxfs>2</cxfs>"+
												"<cxtj>"+dataJson.data.iqseqno+"</cxtj>"+
												"</body>"+
												"</business>";
							//	alert(searchparm);
								ww =  sk.Operate(globalparam);
								
								ret = sk.Operate(searchparm);
			...
			/*					ret ="<?xml version=\"1.0\" encoding=\"gbk\"?>"+
								...
								"</body>"+
								"</business>";*/
								data.data[0].processMsg=ret;
								retpapiao(data.data[0]);
