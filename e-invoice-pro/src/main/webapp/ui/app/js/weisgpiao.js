...
function wkpiao(page){
	...
	$.myAjax({
		...
		success: function (dataJson) {
			...
			$.myPage(page,{rows:dataJson.count,pagesize:16,targetId:"page1",jump:function(page){
				wkpiao(page);
			}});
		},
		error: function (jqXHR, textStatus, errorThrown) {
...
function jisuan(e,column){
	...
	if(user.entid=="SDYZ"&&subshopid=="0"){
		...
		if(newprice > parseFloat(hidprice)){
			$.myAlert("您开票的单价高于商品的售价,商品售价为"+hidprice+"!");
	/*		if(column=='qty'){
				selectTr.find("input[name='qty']").val('');
				 
			}else{
				selectTr.find("input[name='amt']").val('');
				 
			}*/
			jsprice=false;
		}else if(newprice <parseFloat(hidprice)*0.95){
			$.myAlert("您开票的单价低于商品售价的95折,商品售价为"+hidprice+"!");
	/*		if(column=='qty'){
				selectTr.find("input[name='qty']").val('');
...
function loaddate(){ 
	...
	var defultdate=TimeObjectUtil.longMsTimeConvertToDate(now2.setDate(now2.getDate() - 91));
