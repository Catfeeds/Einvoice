...
function search(page){
	...
	var iqchannel = $("#iqchannel").val();
	var iqstatus = $("#iqstatus").val();
	var iqtaxno = $("#iqtaxno").val();
	var iqgmfname = $("#iqgmfname").val();
	...
	if(iqfplxdm !='' && iqfplxdm!=null && iqfplxdm !='9'){
		data.iqfplxdm=iqfplxdm;
	}
	if(iqchannel !='' && iqchannel!=null && iqchannel !='9'){
		data.iqchannel=iqchannel;
	}
	if(invoicetype !='' && invoicetype!=null && invoicetype !='9'){
		data.invoicetype=invoicetype;
	}
	
	if(iqtaxno !='' && iqtaxno!=null && iqtaxno !='9'){
		data.iqtaxno=iqtaxno;
	}
	if(iqstatus !='' && iqstatus!=null){
		data.iqstatus=iqstatus;
	}
	if(iqgmfname !='' && iqgmfname!=null){
		data.iqgmfname=iqgmfname;
	}
	...
	$.myAjax({
		url:$.getService("ui","getHnyjHeadByAdmin"),
		...
		success: function (dataJson) {
			...
			for (var i=0;i<dataJson.data.length;i++){
				...
				if(dataJson.data[i].lxdm=="025"){
					dataJson.data[i].lxdm="卷票";
				}else if(dataJson.data[i].lxdm=="026"){
					dataJson.data[i].lxdm="电子票";
				}else if(dataJson.data[i].lxdm=="004"){
					dataJson.data[i].lxdm="专票";
				}else if(dataJson.data[i].lxdm=="007"){
					dataJson.data[i].lxdm="普票";
				}
				
				if(dataJson.data[i].iqdate != ''){
					dataJson.data[i].iqdate=TimeObjectUtil.longMsTimeConvertToDateTime(dataJson.data[i].iqdate);
				}
			}
...
function excel(){
	var form = $("<form></form>").attr("action", $.getService("ui","exportInvoiceHeadByAdmin")).attr("method", "post");
...
function fp_printforQd(){
	...
	/*	    var submitfrm = document.form1;
		   ...
		   // document.body.appendChild(submitfrm);
			...
			if(dataJson.data.invque.data.taxprintinfo.fpprintUrl!=''&&dataJson.data.invque.data.taxprintinfo.fpprintUrl!=null){
			$.ajax({
                url:dataJson.data.invque.data.taxprintinfo.qdprintUrl,
                type:"post",
                data:"templetName="+dataJson.data.invque.data.taxprintinfo.qdtempletName+"&printerName="+dataJson.data.invque.data.taxprintinfo.qdprinterName+"&data="+encodeURIComponent(datavalue),
                success:function(data){
                     alert(data.msg);
                },
                error:function(e){
