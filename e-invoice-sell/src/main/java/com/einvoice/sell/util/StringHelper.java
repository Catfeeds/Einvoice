...
//	  		 System.out.println(rBlank.length());
	  ...
	  /*
	   * 格式发字符串,示例：str= abc ({0},'{1}','{2}','{3}'),param={"111","222","333","444"}
	   * 
	   */
		...
		*/
		...
		public static void CutStr(String SubStr, String S, StringBuffer S1, StringBuffer S2)
		{
			...
			if (i >= 0)
			{
				...
				S2.append(S.substring(i + SubStr.length()).trim()); //原CSharpToJava代码问题    钱海兵    2011-03-29
			}
			else
			{
				S1.append(S.trim());
		...
		/**
		 * 标识符替换     钱海兵      2011-07-13
		 * @param source    替换源
		 * @param keyword   查找关键字
		 * @param keyset    替换字符串
		 * @param ignoreCase是否跳过大小写
		 * @return
		 */
		public static String ReplaceIdentify(String source,String keyword,String keyset,Boolean ignoreCase){
			...
			if (ignoreCase){
				...
				//修复一个大小写导致不能正常查询到历史单据的问题 Kyo 20110721
